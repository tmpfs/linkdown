 ...
 */
function printJson(item, result) {
  result = {
    url: item.url,
    result: result
  }
  /* istanbul ignore next: keep the test runner output clean */
...
function fetch(info/*, req, crawler*/) {
  ...
  return function onFetchComplete(item, buf) {
    ...
    if(buf && buf.length) {
      ...
      // TODO: support concat of remaining args, ie: -Xss512M
      ...
      if(!userFormat && result && format === 'json') {
        ...
        if(result && result.messages) {
          if(!result.messages.length) {
            ...
            if(json) {
              printJson(item, result);
            }else{
              printer.call(this, result, map);
            }

            if(abort) {
              return this.raise(this.errors.EVALIDATE_ABORT); 
            }
          }
        }
      ...
      /* istanbul ignore next: suppress output in tests */
        ...
        if(abort && (pass instanceof Error)) {
          return this.raise(this.errors.EVALIDATE_ABORT); 
        }
      }
