...
function validate(info, req, next) {
  var uri = info.args[0]
    , fetcher = require('./fetch-complete')
    , parsed
    , jar = this.jar || process.env.NU_VALIDATOR_JAR
    , java = 'java'
    , cmd = 'command -v ' + java;

  // TODO: allow -Xss512M configuration for java invocation
  ...
  var onFetchComplete = fetcher.call(this, info, req, crawler);
