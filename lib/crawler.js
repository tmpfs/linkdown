...
function getCrawler(url, info, req) {
  ...
  function onConstrainPort(url, item) {
    //console.log('%s %s %s', host, port, typeof(port));
    //if(url.port === 80) {
      //console.dir(url); 
    //}

    //console.log('parsed url: ' + url.port);
    //console.dir(url);
    //console.log('item.port: ' + item.port);
    //console.dir(item);
    //
    //if(url.host === host && parseInt(url.port) !== port) {
      //this.log.debug('got port mismatch on same domain %s:%s', url.host, url.port); 
    //}

    this.log.debug('same domain %s:%s (%s:%s)', url.host, url.port, host, port);

    if((url.host === host && parseInt(url.port) !== port)
      || (item.host === host && parseInt(item.port) !== port)) {
