...
function summary(req, cb) {
  ...
  function info() {
    ...
    }else if(def.method === 'complete'){
      queue.countItems({fetched: true}, function(error, size) {
        result[def.field] = size;
        info(); 
      })
    }else if(def.method === 'errors'){
      queue.countItems({status: 'failed'}, function(error, size) {
        result[def.field] = size;
        queue.countItems({status: 'failed'}, function(error, size) {
          result[def.field] += size;
          info(); 
