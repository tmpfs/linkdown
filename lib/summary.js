...
function summary(req) {
  ...
  function onStat(item) {
    this.log.info(
      ...
      ansi(item.format(Math.round(queue.min(item.field) || 0)))[item.color],
      ansi(item.format(Math.round(queue.max(item.field) || 0)))[item.color],
      ansi(item.format(Math.round(queue.avg(item.field) || 0)))[item.color]);
  }
