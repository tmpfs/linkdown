...
function builder(items, opts) {
  ...
  function walk(nodes) {
    ...
    if(opts.pluck && opts.pluck.length) {
      output = output || {nodes: []}; 
    }

    for(var i = 0;i < nodes.length;i++) {
      if(opts.pluck && ~opts.pluck.indexOf(nodes[i].pathname)) {
        output.nodes = output.nodes.concat(nodes[i].nodes);
      }
  ...
  if(opts.sort || opts.pluck) {
    for(var k in map) {
      walk(map[k].nodes); 

      // overwrite nodes when --pluck matches
      if(output && output.nodes.length) {
        map[k].nodes = output.nodes;
      }
    } 
  }
